...
public class ScanTaskQueue
{
    ...
    private readonly ConcurrentDictionary<int, bool> _queuedFrames = new(); // Track which frames have tasks queued
    ...
    public bool TryEnqueue(ScanTask task)
    {
        ...
        if (_queuedFrames.ContainsKey(task.FrameIndex))
        {
        ...
        _queuedFrames.TryAdd(task.FrameIndex, true);
        _queue.Enqueue(task);
    ...
    public async Task<ScanTask?> DequeueAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _queuedFrames.TryRemove(task.FrameIndex, out _); // Remove frame from tracked set
